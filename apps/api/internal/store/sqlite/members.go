...
func (s *Store) ListWorkspaceMemberPage(ctx context.Context, workspaceID, actorUserID string, page store.WorkspaceMemberPageRequest) (store.WorkspaceMemberPage, error) {
	...
	totalCount, totalByRole, err := sqliteWorkspaceMemberCounts(ctx, tx, workspaceID, req)
	if err != nil {
		return store.WorkspaceMemberPage{}, err
	}
	...
	return sqliteWorkspaceMemberPageFromRows(workspaceID, req, totalCount, totalByRole, rows)
}

func sqliteWorkspaceMemberCounts(ctx context.Context, tx *sql.Tx, workspaceID string, req store.WorkspaceMemberPageRequest) (*int, *store.WorkspaceMemberRoleCounts, error) {
	if req.Cursor != "" {
		return nil, nil, nil
	}
	qtx := storedb.New(tx)
