...
WHERE wm.workspace_id = sqlc.arg(workspace_id)
  ...
  AND (CAST(sqlc.arg(search_query) AS TEXT) = '' OR instr(lower(u.display_name), CAST(sqlc.arg(search_query) AS TEXT)) > 0 OR instr(lower(u.handle), CAST(sqlc.arg(search_query) AS TEXT)) > 0)
  AND (
