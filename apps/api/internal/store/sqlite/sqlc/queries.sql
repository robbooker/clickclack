...
-- name: ListWorkspaceUploadStoragePaths :many
SELECT storage_path
FROM uploads
...
WHERE wm.workspace_id = sqlc.arg(workspace_id)
  ...
  AND (CAST(sqlc.arg(search_query) AS TEXT) = '' OR instr(wm.sort_name, CAST(sqlc.arg(search_query) AS TEXT)) > 0 OR instr(wm.sort_handle, CAST(sqlc.arg(search_query) AS TEXT)) > 0)
  AND (wm.role_sort, wm.sort_name, wm.sort_handle, wm.user_id) > (
    CAST(sqlc.arg(cursor_role_sort) AS INTEGER),
...
FROM workspace_members wm
WHERE wm.workspace_id = sqlc.arg(workspace_id)
  AND (instr(wm.sort_name, sqlc.arg(search_query)) > 0 OR instr(wm.sort_handle, sqlc.arg(search_query)) > 0);
...
-- name: GetEventDeliveryAttemptCursor :one
SELECT created_at
...
WHERE eda.subscription_id = sqlc.arg(subscription_id)
  AND (
    ...
    OR eda.created_at < CAST(sqlc.arg(before_created_at) AS TEXT)
    OR (
      eda.created_at = CAST(sqlc.arg(before_created_at) AS TEXT)
