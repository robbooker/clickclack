...
func (s *Store) listMessagePage(ctx context.Context, scope messagePageScope, req store.MessagePageRequest) (store.MessagePage, error) {
	...
	case messagePageBefore:
		messages, err = s.queryScopedMessages(ctx, scope, "m.channel_seq < ?", []any{*req.BeforeSeq}, "DESC", req.Limit)
		reverseMessages(messages)
	case messagePageAfter:
		messages, err = s.queryScopedMessages(ctx, scope, "m.channel_seq > ?", []any{*req.AfterSeq}, "ASC", req.Limit)
...
func (s *Store) queryMessagesAround(ctx context.Context, scope messagePageScope, cursor int64, limit int) ([]store.Message, error) {
	...
	left, err := s.queryScopedMessages(ctx, scope, "m.channel_seq <= ?", []any{cursor}, "DESC", limit)
	if err != nil {
		return nil, err
	}
	takeLeft := min(len(left), leftLimit)
	right, err := s.queryScopedMessages(ctx, scope, "m.channel_seq > ?", []any{cursor}, "ASC", rightLimit+(leftLimit-takeLeft))
	if err != nil {
		return nil, err
	}
...
func (s *Store) buildMessagePage(ctx context.Context, scope messagePageScope, messages []store.Message) (store.MessagePage, error) {
	...
	hasOlder, err := s.hasScopedMessage(ctx, scope, "m.channel_seq < ?", page.OldestSeq)
	if err != nil {
		return store.MessagePage{}, err
	}
	hasNewer, err := s.hasScopedMessage(ctx, scope, "m.channel_seq > ?", page.NewestSeq)
	if err != nil {
		return store.MessagePage{}, err
	}
...
func (s *Store) hasScopedMessage(ctx context.Context, scope messagePageScope, cursorWhere string, cursor int64) (bool, error) {
	...
	rows, err := s.db.QueryContext(ctx, `
		SELECT 1
		FROM messages m
		WHERE `+scope.where+` AND `+cursorWhere+`
		LIMIT 1`, args...)
	if err != nil {
		return false, err
	}
