...
WHERE eda.subscription_id = ?1
  AND (
    CAST(?2 AS TEXT) = ''
    OR eda.created_at < CAST(?3 AS TEXT)
    OR (
      eda.created_at = CAST(?3 AS TEXT)
      AND eda.id < ?2
    )
  )
...
func (q *Queries) ListPendingUploadCleanups(ctx context.Context, rowLimit int64) ([]PendingUploadCleanup, error) {
