...
WHERE wm.workspace_id = $1
  AND (position(CAST($2 AS TEXT) in lower(u.display_name)) > 0 OR position(CAST($2 AS TEXT) in lower(u.handle)) > 0)
`
...
WHERE wm.workspace_id = $1
  AND wm.role = $2
  AND (position(CAST($3 AS TEXT) in lower(u.display_name)) > 0 OR position(CAST($3 AS TEXT) in lower(u.handle)) > 0)
`
